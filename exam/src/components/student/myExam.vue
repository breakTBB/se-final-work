  ...
  <div id="myExam">
    ...
    <div class="wrapper">
      ...
      <ul class="paper" v-loading="loading">
        <li
          class="item"
          v-for="(item, index) in this.pagination.records"
          :key="index"
        >
    ...
    getExamInfo() {
      this.$axios(
        ...
        .then((res) => {
          ...
          this.pagination.records = this.pagination.records.filter((item) => {
            var dt = new Date(
              Number(item.examDate.substr(0, 4)),
              Number(item.examDate.substr(5, 2)) - 1,
              Number(item.examDate.substr(8, 2))
            );
            console.log(
              item.examDate.substr(0, 4) +
                item.examDate.substr(5, 2) +
                item.examDate.substr(8, 2)
            );
            console.log(dt);
